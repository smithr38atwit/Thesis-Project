...
class DCGCriticNS:
    ...
    def __init__(self, scheme, args):
        ...
        self.utility_fun = [
            self._mlp(self.args.hidden_dim, args.cg_utilities_hidden_dim, self.n_actions) for _ in range(self.n_agents)
        ]
    ...
    def annotations(self, ep_batch, t, compute_grads=False, actions=None):
        ...
        with th.no_grad() if not compute_grads else contextlib.suppress():
            ...
            for i, ag in enumerate(self.agents):
                self.hidden_states[i] = ag(agent_inputs[:, i, :], self.hidden_states[i])[1].view(
    ...
    def single_payoff(self, payoff_fun, edge, hidden_states):
        ...
        inputs = th.stack(
            [
                th.cat([hidden_states[self.edges_from[edge]], hidden_states[self.edges_to[edge]]], dim=-1),
    ...
    def _set_edges(self, edge_list):
        ...
        self.edges_n_in = torch_scatter.scatter_add(
            src=self.edges_to.new_ones(len(self.edges_to)), index=self.edges_to, dim=0, dim_size=self.n_agents
        ) + torch_scatter.scatter_add(
            src=self.edges_to.new_ones(len(self.edges_to)), index=self.edges_from, dim=0, dim_size=self.n_agents
        )
