...
def load_results(path, metric):
    ...
    for file in metrics_files:
        ...
        common_reward = False  # config["common_reward"]
        reward_scalarisation = "sum"  # config["reward_scalarisation"]

        data[(str(env_args), env_name, common_reward, reward_scalarisation)][alg_name][str(config)].append(
            (config, steps, values)
        )
    return data
...
def main():
    ...
    data = {
        env_key: {alg_name: aggregate_results(alg_data) for alg_name, alg_data in env_data.items()}
        for env_key, env_data in data.items()
    }
    if args.smoothing_window is not None:
        data = {
            env_key: {alg_name: smooth_data(alg_data, args.smoothing_window) for alg_name, alg_data in env_data.items()}
            for env_key, env_data in data.items()
        }
    data = {
        env_key: {alg_name: shorten_config_names(alg_data) for alg_name, alg_data in env_data.items()}
        for env_key, env_data in data.items()
    }
    if args.best_per_alg:
        ...
        for env_key, env_data in data.items():
            for alg_name, alg_data in env_data.items():
                ...
                best_data[env_key][alg_name] = {best_config_key: alg_data[best_config_key]}
        data = best_data
    plot_results(
        data,
        args.metric,
